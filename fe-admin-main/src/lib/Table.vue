<template>
  <Table>
    <TableHeader>
      <TableRow>
        <TableHead v-for="column in columns">
          <Button v-if="column.sortKey ? true : false" variant="secondary"
    ...
    <TableBody>
      <component v-if="!isLoading" v-for="subject in subjects" :is="row" :data="subject" @clickDelete="
        (data: any) => {
...
defineEmits(["clickDelete", "clickUpdate", "clickAction", "clickSorting", "clickActive"]);
</script>
