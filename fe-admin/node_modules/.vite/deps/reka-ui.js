...
function refAutoReset2(defaultValue, afterMs = 1e4) {
...
function useTypeahead(callback) {
  const search = refAutoReset2("", 1e3);
...
var _sfc_main71 = defineComponent({
  ...
  setup(__props) {
    ...
    const rootContext = injectListboxRootContext();
    const isClickFocus = refAutoReset2(false, 10);
    return (_ctx, _cache) => {
      return openBlock(), createBlock(unref(CollectionSlot), null, {
        default: withCtx(() => [
          createVNode(unref(Primitive), {
...
function useGraceArea(triggerElement, containerElement) {
  const isPointerInTransit = refAutoReset2(false, 300);
  const pointerGraceArea = ref(null);
  const pointerExit = createEventHook2();
...
function usePressedHold(options) {
  ...
  const timeout = ref();
  const triggerHook = createEventHook2();
...
var _sfc_main246 = defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    const props2 = __props;
    const emits = __emit;
    const { disabled, min, max, step, formatOptions, id, locale: propLocale } = toRefs(props2);
    const modelValue = useVModel(props2, "modelValue", emits, {
      defaultValue: props2.defaultValue,
      passive: props2.modelValue === void 0
    });
...
var _sfc_main306 = defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    const props2 = __props;
    const emits = __emit;
    const { min, max, step, minStepsBetweenThumbs, orientation, disabled, dir: propDir } = toRefs(props2);
    const dir = useDirection(propDir);
    const { forwardRef, currentElement } = useForwardExpose();
    const isFormControl = useFormControl(currentElement);
...
var _sfc_main358 = defineComponent({
  ...
  setup(__props) {
    ...
    const isPointerInTransitRef = ref(false);
    const { start: startTimer, stop: clearTimer } = useTimeoutFn2(() => {
