<template>
  <Dialog
    ...
    @update:open="recommendStore.createDialogIsOpen = $event"
  >
    <DialogContent>
      <DialogHeader>
      ...
      <div class="flex items-center gap-4">
        <Label class="text-right w-1/4">Active From</Label>
        <Input
          type="datetime-local"
          v-model="recommendStore.selectedData.active_from"
        />
      </div>
      <div class="flex items-center gap-4">
        <Label class="text-right w-1/4">Active To</Label>
        <Input
          type="datetime-local"
          v-model="recommendStore.selectedData.active_to"
        />
      </div>
      <DialogFooter class="sm:justify-end">
        ...
        <Button
          @click="
            async () => {
              await recommendStore.createRecommend(recommendStore.selectedData);
