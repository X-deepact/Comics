<template>
  <div class="p-8">
    <Component title="Genre" :pageSize="genreStore.page_size" @clickAdd="() => (genreStore.createDialogIsOpen = true)"
      @clickRefresh="
        () => {
      ...
      " />
    <TableComponent :subjects="genreStore.genreData" :columns="ColumnNames" :isLoading="genreStore.isLoading"
      :row="GenreRow" @clickDelete="
        (data: any) => {
      ...
      "
      @clickSorting="(data: any) => { genreStore.sortBy = data; genreStore.setSorting(); genreStore.getGenreData(); }" />

    <Pagination :currentPage="genreStore.current_page" :totalItems="genreStore.totalItems" :totalPages="genreStore.totalItems % genreStore.page_size === 0
